...
- **身材比例調控系統實裝（解決頭身腿比例不完美問題）**：
  - **根本原因排查**：由於參考圖多為臉部特寫裁切，Vision Cross-Attention 在無解剖比例約束下容易將頭部放大（約 6 頭身），導致軀幹緊縮與小短腿。
  - **五大身材比例預設（PROPORTION_PRESETS）**：
    - `golden_8head`（八頭身超模（黃金比例·小頭長腿）★推薦預設）：精緻小頭小臉、纖長頸線、自然高腰、極致纖細長腿，建立標準八頭身黃金比例。
    - `supermodel_9head`（九頭身極致修長（伸展台超模·極致美腿））：極致修長伸展台高訂超模比例。
    - `natural_balanced`（自然勻稱（標緻和諧·真實美感））：真實生活感標緻比例。
    - `petite_slender`（嬌小苗條（精巧骨架·優雅比例））：小骨架精緻比例。
    - `default`（照寫法預設）：保留原始風格設定。
  - **智慧取景感知（Framing-Aware）**：在半身（`waist`）或特寫（`head`）取景時自動剔除腿部語句，避免模型在非全身畫面硬塞腿部造成畸變；七分與全身時完整釋放長腿約束。
  - **解剖負向提示詞強化**：加入 `oversized head, big head, short legs, long torso, disproportionate body, stubby limbs` 杜絕大頭與小短腿。
  - **GUI 介面完整整合**：介面第一排新增「身材比例（頭身腿黃金比）」下拉選單，預設啟用八頭身超模，即時預覽與生圖全面生效。
- **ComfyUI 獨立環境準備**：
