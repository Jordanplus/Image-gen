...
## 常駐 24/7（launchd）
`~/Library/LaunchAgents/com.imagegen.server.plist`（記得改 `<你的使用者名>` 與路徑、Python 路徑）：
```xml
...
<plist version="1.0"><dict>
  ...
  <array>
    ...
    <string>server:app</string><string>--host</string><string>0.0.0.0</string>
...
```
```bash
launchctl load ~/Library/LaunchAgents/com.imagegen.server.plist   # 啟用（崩潰自動重拉、開機自啟）
# 不睡眠：sudo pmset -a sleep 0 ；System Settings → Energy 開「停電後自動開機」
```
